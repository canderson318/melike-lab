...
for w=1:length(window_sets)
    ...
    for i=1:length(data_names)
        ...
        if exist(summary_path)
            ...
            fprintf("Optimization summary not found at %s", summary_path)
        end
...
%     if ~ exist(fullfile(in_dir, "plots", window))
%         mkdir(fullfile(in_dir, "plots", window))
%     end
...
%     saveas(gcf, fullfile(in_dir, "plots", window, 'rmse_against_interval_start.png'))
...
%     colorbar
%     hold off 
%     saveas(gcf, fullfile(in_dir, "plots", window, 'gamma_against_interval_start_rmse_filled.png'))
...
% saveas(gcf, fullfile(in_dir, "plots", 'gammas_at_diff_start_times.png'))
